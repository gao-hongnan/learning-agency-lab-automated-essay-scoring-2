...
from .models import OrdinalLogisticModel
...
class CumulativeLinkLoss(nn.Module):
    ...
    def __init__(self, num_classes: int = 6, init_cutpoints: str = 'ordered', reduction: str = 'elementwise_mean',
                 class_weights: Optional[torch.Tensor] = None) -> None:
        super().__init__()
        self.class_weights = class_weights
        self.reduction = reduction
        self.convert_model = OrdinalLogisticModel()

    # for regression inputs
    
    def forward(self, preds: torch.Tensor,
                labels: torch.Tensor) -> torch.Tensor:

        y_pred = self.convert_model(preds)
        y_true = labels.to(torch.long).reshape((-1, 1))
